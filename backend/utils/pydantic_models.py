...
class DocumentSpec(BaseModel):
    """A required response document generated by the Scenario Analyst."""
    doc_id: str
    title: str
    owner_agent_id: str = Field(description="role_key of the agent responsible")
    deadline_hours: int = Field(default=72, description="Hours from session start")
...
class DocumentDraftSection(BaseModel):
    """A single draft section of a response document, written by an agent."""
    content: str
    by: str = Field(description="Character name of the drafting agent")
    status: str = Field(default="draft", description="draft | review | final")
    turn: int = Field(default=0, description="Turn number when written")
    updated_at: Optional[str] = None


class DeadlineRisk(BaseModel):
    """A flagged deadline risk from an agent."""
    risk_id: str
    deadline_label: str
    risk_note: str
    hours_remaining: Optional[float] = None
    flagged_by: str = Field(description="Character name of the flagging agent")
...
class AgentConfig(BaseModel):
    ...
    blind_spot: str = ""
    documents_responsible: list[str] = Field(default_factory=list)
...
class ScenarioSpec(BaseModel):
    ...
    # ── Document specs (v2.0, optional — generated when scenario supports it)
...
class CrisisSessionModel(BaseModel):
    ...
    # ── Document engine fields (v2.0) ────────────────────────────────
    required_documents: list[dict] = Field(default_factory=list)
    document_drafts: dict = Field(default_factory=dict)
    deadline_risks: list[dict] = Field(default_factory=list)
    uploaded_context: str = Field(default="", description="Extracted text from uploaded files")
